  ...
  const statCards = isClient
    ? [
        ...
        },
        {
          ...
          sub: `${stats.unpaidCount ?? 0} Outstanding`,
          icon: DollarSign,
          dark: true,
        },
        {
          label: "UPCOMING APPTS",
          value: String(upcoming48.length).padStart(2, "0"),
          sub: upcoming48[0] ? `Next: ${upcoming48[0].time}` : "Clear schedule",
          icon: CalendarDays,
          dark: false,
        },
        {
    ...
    : [
        ...
        {
          label: "UPCOMING (48H)",
          value: String(stats.upcoming48Count ?? 0).padStart(2, "0"),
          sub: upcoming48[0] ? `Next: ${upcoming48[0].time}` : "Clear schedule",
          icon: CalendarDays,
          dark: false,
        },
        {
          label: "PENDING TASKS",
          value: String(stats.pendingTaskCount ?? 0).padStart(2, "0"),
          sub:
      ...
      >
        ...
        {isUnlinkedClient && (
          <View style={s.unlinkedBanner}>
            <View style={s.unlinkedBannerLeft}>
              <Text style={s.unlinkedBannerTitle}>
                Account Pending Connection
              </Text>
              <Text style={s.unlinkedBannerBody}>
                Your account isn't linked to a freelancer yet. Once your
                freelancer invites{" "}
                <Text style={{ fontWeight: "900" }}>{user?.email}</Text>, your
        ...
        <View style={s.statsGrid}>
          {statCards.map((card, i) => {
            ...
            return (
              <View
                key={i}
                style={[s.statCard, card.dark && s.statCardDark, shadow]}
              >
                <View style={[s.statIconBox, card.dark && s.statIconBoxDark]}>
                  <Icon
                    size={17}
                    color={card.dark ? C.secondaryContainer : C.primary}
                  />
                </View>
                <Text style={[s.statLabel, card.dark && s.statLabelDark]}>
                  {card.label}
                </Text>
                <Text
                  style={[s.statValue, card.dark && s.statValueDark]}
                  numberOfLines={1}
                >
                  {card.value}
                </Text>
                <Text style={[s.statSub, card.dark && s.statSubDark]}>
                  {card.sub}
                </Text>
              </View>
            );
        ...
        <View style={s.section}>
          <View style={s.sectionHeader}>
            ...
            <TouchableOpacity
              onPress={() =>
                navigation.navigate(isClient ? "Booking" : "Calendar")
              }
            >
            ...
            {upcoming48.length > 0 ? (
              upcoming48.map((apt, i) => {
                ...
                return (
                  <View
                    key={apt._id}
                    style={[s.aptRow, i > 0 && s.aptRowBorder]}
                  >
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <Text style={s.aptTitle} numberOfLines={1}>
                        {apt.title}
                      </Text>
                    </View>
              ...
              <View style={s.empty}>
                ...
                <Text style={s.emptyText}>
                  Clean 48 hours. Use your time wisely.
                </Text>
              </View>
            )}
          </View>
        ...
        <View style={s.section}>
          <View style={s.sectionHeader}>
            <Text style={s.sectionTitle}>
              {isClient ? "MY PROJECTS" : "MY TO-DO LIST"}
            </Text>
            <TouchableOpacity
              onPress={() =>
                navigation.navigate(isClient ? "Projects" : "Tasks")
              }
            >
          ...
          <View style={[s.card, shadow]}>
            {isClient ? (
                ...
                projectProgress.slice(0, 4).map((proj, i) => (
                  ...
                  >
                    <View
                      style={[
                        s.taskDot,
                        proj.status === "Overdue" && s.taskDotOverdue,
                      ]}
                    />
                    <View style={{ flex: 1 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      <View style={s.taskMeta}>
                        ...
                        <View style={s.taskTimeBadge}>
                          <Clock
                            size={10}
                            color={isOverdue ? C.error : C.outline}
                          />
                          <Text
                            style={[
                              s.taskTimeText,
                              isOverdue && { color: C.error },
                            ]}
                          >
                            {task.due
              ...
              <View style={s.empty}>
                <RefreshCcw size={28} color={C.outlineVar} />
                <Text style={s.emptyText}>
                  All tasks cleared. Architecture is complete.
                </Text>
              </View>
            )}
            <TouchableOpacity
            ...
            <View style={[s.card, shadow]}>
              {recentInvoices.map((inv, i) => {
                const label = isClient
                  ? (inv.freelancerId?.businessName ??
                    inv.freelancerId?.name ??
                    "Freelancer")
                ...
                return (
                  <View
                    key={inv._id}
                    style={[s.invRow, i > 0 && s.invRowBorder]}
                  >
