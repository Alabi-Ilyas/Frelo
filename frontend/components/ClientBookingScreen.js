...
import {
  Calendar, PlusCircle, Clock, Bell, ChevronLeft, ChevronRight,
} from "lucide-react-native";
import {
  getClientAppointments, getMyFreelancers,
  getAvailableSlots, bookAppointment, cancelClientAppointment,
} from "../api/apiCalls";
...
export default function ClientBookingScreen() {
    ...
    Alert.prompt(
      ...
      async (title) => {
        ...
        try {
          ...
          Alert.alert("Error", e.response?.data?.message ?? "Could not book appointment.");
  ...
  const filteredAppts = appointments.filter(a => {
    ...
    if (apptFilter === "upcoming") return d >= today && a.status !== "Cancelled";
    if (apptFilter === "past")     return d < today || a.status === "Completed" || a.status === "No-show";
    return true;
      ...
      <View style={s.tabRow}>
        {[
          ...
          >
            ...
            {key === "appointments" && appointments.filter(a => new Date(a.date) >= today && a.status === "Confirmed").length > 0 && (
              <View style={s.tabBadge}>
                <Text style={s.tabBadgeText}>
                  {appointments.filter(a => new Date(a.date) >= today && a.status === "Confirmed").length}
                </Text>
              </View>
            )}
          </TouchableOpacity>
        ...
        >
          ...
          <View style={s.filterRow}>
            {["upcoming", "past", "all"].map(f => (
              ...
              >
                <Text style={[s.filterText, apptFilter === f && s.filterTextActive]}>
          ...
          {filteredAppts.length === 0 ? (
            <View style={s.empty}>
              <View style={s.emptyIcon}><Bell size={32} color={C.outline} /></View>
              <Text style={s.emptyTitle}>No appointments</Text>
              <Text style={s.emptyBody}>
                {apptFilter === "upcoming" ? "No upcoming meetings. Book one below." : "Nothing here yet."}
              </Text>
              {apptFilter === "upcoming" && (
                <TouchableOpacity style={s.emptyBtn} onPress={() => setTab("new")}>
            ...
            filteredAppts.map((apt, idx) => {
              ...
              const isDone = apt.status === "Cancelled" || apt.status === "Completed" || apt.status === "No-show";
              return (
                <TouchableOpacity
                  key={apt._id}
                ...
                >
                  <View style={s.apptTop}>
                    ...
                    <View style={[s.statusBadge, { backgroundColor: cfg.bg }]}>
                      <Text style={[s.statusText, { color: cfg.text }]}>{apt.status}</Text>
                    </View>
                  </View>
                  <Text style={s.apptTitle} numberOfLines={1}>{apt.title}</Text>
                  <Text style={s.apptMeta}>
                    <Clock size={11} color={C.outline} /> {fmt12(apt.time)} · {apt.duration} min
                  </Text>
                  <Text style={s.apptFreelancer}>
                    with {apt.freelancerId?.businessName ?? apt.freelancerId?.name ?? "Freelancer"}
                  </Text>
                  {apt.meetingLink && (
        ...
        <ScrollView contentContainerStyle={s.container} showsVerticalScrollIndicator={false}>
          ...
          {freelancers.length === 0 ? (
            <View style={[s.infoBox, shadow]}>
              <Text style={s.infoText}>You're not connected to any freelancer yet.</Text>
            </View>
          ) : (
            <View style={s.flRow}>
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginTop: 8 }}>
                {freelancers.map(fl => (
                  <TouchableOpacity
                    key={fl._id}
                    style={[s.flPill, selectedFl?._id === fl._id && s.flPillActive]}
                    onPress={() => { setSelectedFl(fl); setSelectedDate(null); setSlots([]); }}
                  >
                    <Text style={[s.flPillText, selectedFl?._id === fl._id && s.flPillTextActive]}>
          ...
          <View style={[s.calCard, shadow]}>
            <View style={s.calHeader}>
              ...
              <View style={s.calNav}>
                <TouchableOpacity style={s.calNavBtn} onPress={() => setCurrentDate(new Date(year, month - 1, 1))}>
              ...
              {Array.from({ length: daysInMonth }).map((_, i) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => !past && selectedFl && handleSelectDate(new Date(year, month, day))}
                    disabled={past || !selectedFl}
                    activeOpacity={0.7}
                  >
                    <Text style={[
                      s.calCellText,
          ...
          <View style={[s.slotsCard, shadow]}>
            ...
            {!selectedDate || !selectedFl ? (
              <Text style={s.slotsHint}>
                {!selectedFl ? "Select a freelancer above." : "Select a date to see available slots."}
              </Text>
            ) : loadingSlots ? (
              <ActivityIndicator color={C.primary} style={{ marginVertical: 24 }} />
            ) : slots.length === 0 ? (
              <Text style={s.slotsHint}>No slots available. Try a different date.</Text>
            ) : (
              <>
                <Text style={s.slotsMeta}>{slotDuration}-min sessions · {slots.length} slot{slots.length !== 1 ? "s" : ""} open</Text>
...
const s = StyleSheet.create({
  ...
  tabRow:         { flexDirection: "row", borderBottomWidth: 1, borderBottomColor: C.outlineVar, backgroundColor: C.background },
  ...
  tabText:        { fontSize: 10, fontWeight: "900", color: C.outline, letterSpacing: 1 },
  tabTextActive:  { color: C.primary },
  tabBadge:       { backgroundColor: C.primary, borderRadius: 10, paddingHorizontal: 5, paddingVertical: 1 },
  tabBadgeText:   { color: "#fff", fontSize: 9, fontWeight: "900" },

  container: { padding: 16, paddingBottom: 100 },

  filterRow:       { flexDirection: "row", gap: 8, marginBottom: 16 },
  filterPill:      { paddingHorizontal: 14, paddingVertical: 7, borderRadius: 20, backgroundColor: C.card, borderWidth: 1, borderColor: C.outlineVar },
  ...
  apptCard:     { backgroundColor: C.card, borderRadius: 22, padding: 18, marginBottom: 12, borderWidth: 1, borderColor: C.outlineVar },
  apptCardDone: { opacity: 0.6 },
  apptTop:      { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 },
  dateBadge:    { width: 52, height: 52, borderRadius: 16, backgroundColor: C.surfaceLow, alignItems: "center", justifyContent: "center", borderWidth: 1, borderColor: C.outlineVar },
  dateBadgeMon: { fontSize: 9, fontWeight: "900", color: C.primary, letterSpacing: 0.5 },
  dateBadgeDay: { fontSize: 22, fontWeight: "900", color: C.primary, lineHeight: 26 },
  statusBadge:  { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 10 },
  statusText:   { fontSize: 9, fontWeight: "900" },
  apptTitle:    { fontSize: 16, fontWeight: "800", color: C.onSurface, marginBottom: 6 },
  ...
  linkDotCircle:{ width: 6, height: 6, borderRadius: 3, backgroundColor: C.primary },
  linkDotText:  { fontSize: 10, fontWeight: "900", color: C.primary, letterSpacing: 0.5 },
  ...
  emptyBtn:    { flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: C.primary, paddingHorizontal: 24, paddingVertical: 14, borderRadius: 14 },
  emptyBtnText:{ color: "#fff", fontSize: 10, fontWeight: "900", letterSpacing: 1 },

  infoBox:  { backgroundColor: C.surfaceLow, borderRadius: 16, padding: 16, marginBottom: 16 },
  infoText: { fontSize: 13, color: C.onSurfaceVar, fontWeight: "600" },

  flRow:         { marginBottom: 16 },
  flLabel:       { fontSize: 9, fontWeight: "900", color: C.onSurfaceVar, letterSpacing: 1.5 },
  flPill:        { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 12, backgroundColor: C.card, borderWidth: 1, borderColor: C.outlineVar, marginRight: 8 },
  ...
  calCard:   { backgroundColor: C.card, borderRadius: 22, padding: 18, marginBottom: 16, borderWidth: 1, borderColor: C.outlineVar },
  ...
  calNavBtn: { width: 36, height: 36, borderRadius: 10, backgroundColor: C.surfaceLow, alignItems: "center", justifyContent: "center" },
  calDayRow: { flexDirection: "row", marginBottom: 8 },
  calDayLabel:{ flex: 1, textAlign: "center", fontSize: 9, fontWeight: "900", color: C.outline, letterSpacing: 0.5 },
  ...
  calDot:    { position: "absolute", bottom: 3, width: 4, height: 4, borderRadius: 2, backgroundColor: C.secondaryContainer },

  slotsCard:  { backgroundColor: C.card, borderRadius: 22, padding: 18, borderWidth: 1, borderColor: C.outlineVar },
  slotsTitle: { fontSize: 10, fontWeight: "900", color: C.onSurfaceVar, letterSpacing: 1.5, marginBottom: 12 },
  slotsHint:  { fontSize: 13, color: C.outline, fontWeight: "600", textAlign: "center", paddingVertical: 24 },
  slotsMeta:  { fontSize: 10, color: C.outline, fontWeight: "700", marginBottom: 12 },
  slotsGrid:  { flexDirection: "row", flexWrap: "wrap", gap: 10 },
  slotBtn:    { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 16, paddingVertical: 12, borderRadius: 12, backgroundColor: C.surfaceLow, borderWidth: 1, borderColor: C.outlineVar },
